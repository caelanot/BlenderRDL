...
async def on_ready():
    ...
    await client.change_presence(activity=discord.CustomActivity("Making coffee ☕", emoji=discord.PartialEmoji.from_str("coffee")))
...
@client.tree.command(
    name="randomblend",
    description="Adds level to list of random blends"
)
@commands.has_any_role(*roles)
@app_commands.describe(level="Level to blend")
...
@client.tree.command(
    name="viewqueue",
    description="View levels in queue"
)
@commands.has_any_role(*roles)
...
@client.tree.command(
    name="viewrandom",
    description="View levels in random"
)
@commands.has_any_role(*roles)
...
async def get_blend():
    if client.blend:
        ...
        try:
            ...
            levels = open("random").read().splitlines()
            print(levels)
